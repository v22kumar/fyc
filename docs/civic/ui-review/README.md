...
Note the filename: no `_test` suffix, so `flutter test` never collects it. It
is a camera, not an assertion suite, and in a headless container it does not
always shut down cleanly. Left in the default run it fails CI over screenshots
nobody asked for.
