...
def test_second_innings_rejects_identical_names(client, db):
    ...
    # Confirm the innings actually transitioned — otherwise the 400 below could
    # come from "Match is not in innings break" instead of the name-collision
    # rejection this test means to exercise, and would pass for the wrong reason.
    ms = client.get(f"/api/v1/fixtures/{fid}/cricket", headers=H).json()
    assert ms["match_state"]["status"] == "INNINGS_BREAK"
...
def test_wicket_on_final_legal_ball_of_innings_does_not_require_new_batter(client, db):
    ...
    no replacement batter is coming in and none should be required."""
    H, fid, team_ids = _setup_fixture(client, db)
    init = client.post(f"/api/v1/fixtures/{fid}/cricket/init",
                       json=_init_payload(team_ids[0], overs=1), headers=H)
    p = init.json()["current_players"]

    # 5 dot balls to reach the last ball of the (1-over) innings.
    for _ in range(5):
        r = client.post(f"/api/v1/fixtures/{fid}/cricket/ball", json={
            "striker_id": p["striker_id"], "non_striker_id": p["non_striker_id"],
    ...
    # The 6th and final ball is a wicket — no new_batter_name supplied, and it
    # must still be accepted because the innings ends on this delivery.
    w = client.post(f"/api/v1/fixtures/{fid}/cricket/ball", json={
        "striker_id": p["striker_id"], "non_striker_id": p["non_striker_id"],
        "bowler_id": p["bowler_id"], "runs_batter": 0, "is_wicket": True,
        "wicket_type": "BOWLED", "player_dismissed_id": p["striker_id"],
    ...
    assert w.status_code == 200, w.text
    assert w.json()["match_state"]["status"] == "INNINGS_BREAK"
