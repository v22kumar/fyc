...
async def get_top_news(
    ...
    return await service.get_top_tamil_news(limit=limit)
...
async def get_india_news(
    ...
    if response is not None:
        response.headers["Cache-Control"] = _NEWS_CC
    return await service.get_india_news(limit=limit)


@router.get("/tn-jobs", response_model=list[NewsItemResponse])
async def get_tn_jobs_news(
    ...
    if response is not None:
        response.headers["Cache-Control"] = _NEWS_CC
    return await service.get_tn_jobs_news(limit=limit)


@router.get("/central-jobs", response_model=list[NewsItemResponse])
async def get_central_jobs_news(
    ...
    if response is not None:
        response.headers["Cache-Control"] = _NEWS_CC
    return await service.get_central_jobs_news(limit=limit)


@router.get("/kanyakumari", response_model=list[NewsItemResponse])
async def get_kanyakumari_news(
    ...
    if response is not None:
        response.headers["Cache-Control"] = _NEWS_CC
    return await service.get_kanyakumari_news(limit=limit)
