...
class UserRegister(BaseModel):
    organization_id: UUID
    phone_number: str
    registration_token: Optional[str] = Field(None)
    email: Optional[str] = Field(None)
    date_of_birth: Optional[date] = Field(None, description="Optional — asked later as a profile prompt")
    ...
    def _name_required(cls, v: str) -> str:
        if not v or not v.strip(): raise ValueError("Name is required")
        return v.strip()
